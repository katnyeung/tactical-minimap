...
public interface MarkerResponseDAO<T extends MarkerResponse> extends JpaRepository<T, Long> {
	...
	@Query(value = "SELECT GREATEST(50 - count(1),1) as markerCount FROM marker_response mr INNER JOIN marker m ON mr.marker_id = m.marker_id WHERE m.layer = :layer AND m.status = 'A' AND m.lat BETWEEN :fromLat AND :toLat AND m.lng BETWEEN :fromLng AND :toLng", nativeQuery = true)
